...
// import { UnlockContactSection } from '@/components/listing/unlock-contact-section' /* PAYMENT_DISABLED */
...
export function MobileBottomBar({
  ...
  // const [unlockSheetOpen, setUnlockSheetOpen] = useState(false) /* PAYMENT_DISABLED */
  ...
  return (
    <>
      ...
      >
        ...
        {listingStatus && listingStatus !== 'ACTIVE' && !isOwner ? (
          ...
          <button
            ...
            className="flex shrink-0 items-center gap-2 rounded-xl border border-green-200 bg-green-50 px-4 py-3 text-sm font-semibold text-green-800 transition hover:bg-green-100 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--color-ring)] focus-visible:ring-offset-2"
          >
      ...
      {/* PAYMENT_DISABLED — unlock bottom sheet removed */}
      {/*
      {unlockSheetOpen && interestId && (
        <div className="fixed inset-0 z-[60] flex items-end lg:hidden" role="dialog" aria-modal="true" aria-label="Unlock seller contact">
          <div className="absolute inset-0 bg-black/40" onClick={() => setUnlockSheetOpen(false)} aria-hidden="true" />
          <div className="relative w-full rounded-t-2xl border-t border-[var(--color-border)] bg-[var(--color-background)] p-6">
            <button type="button" onClick={() => setUnlockSheetOpen(false)} className="absolute right-4 top-4 rounded-lg p-1.5 text-[var(--color-muted-foreground)] hover:bg-[var(--color-muted)]" aria-label="Close">✕</button>
            <UnlockContactSection interestId={interestId} listingTitle={listingTitle} onUnlocked={(phone, email) => { setLocalPhone(phone); setLocalUnlocked(true); setUnlockSheetOpen(false); void email }} />
          </div>
        </div>
      )}
      */}
