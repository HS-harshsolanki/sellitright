...
import { type ListingStatus, MOCK_LISTINGS, type MockListing } from '@/lib/mock-data'
import { formatPrice } from '@/lib/format'
import Image from 'next/image'
import Link from 'next/link'
import { useState } from 'react'
import { Eye, LayoutGrid, MapPin, Plus, TrendingUp } from 'lucide-react'

type TabFilter = 'all' | 'active' | 'draft' | 'sold'
...
const PLACEHOLDER_LISTINGS: MockListing[] = [
  ...MOCK_LISTINGS,
  ...(MOCK_LISTINGS[0]
    ? [
        {
      ...
      <div className="p-4">
        ...
        <div className="mt-3 flex gap-2">
          <Link
            ...
            className={cn(
              'flex-1 rounded-lg border border-border py-2 text-center text-xs font-medium text-foreground',
...
function EmptyState({ tab }: { tab: TabFilter }) {
  const messages: Record<TabFilter, { title: string; sub: string }> = {
    ...
    active: {
      ...
      sub: 'Your published listings will appear here.',
    },
    draft: {
      title: 'No drafts',
    ...
    sold: {
      ...
      sub: 'Properties you\'ve marked as sold will appear here.',
    },
  }
  ...
  return (
    <div className="flex flex-col items-center justify-center rounded-2xl border-2 border-dashed border-border py-16 text-center">
      ...
      <Link
        href="/sell"
        className={cn(
          'mt-6 flex items-center gap-2 rounded-xl bg-primary px-5 py-2.5 text-sm font-semibold text-white',
...
export default function DashboardPage() {
  ...
  const filteredListings = PLACEHOLDER_LISTINGS.filter((listing) => {
    if (activeTab === 'all') return true
    return listing.status.toLowerCase() === activeTab
  })

  const stats = {
    total: PLACEHOLDER_LISTINGS.length,
    active: PLACEHOLDER_LISTINGS.filter((l) => l.status === 'ACTIVE').length,
    views: PLACEHOLDER_LISTINGS.reduce((sum, l) => sum + l.viewCount, 0),
  }
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        ...
        <Link
          href="/sell"
          className={cn(
            'hidden items-center gap-2 rounded-xl bg-primary px-4 py-2.5 text-sm font-semibold text-white sm:flex',
            'transition-colors hover:bg-primary/90 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring',
          )}
        >
          <Plus className="h-4 w-4" />
